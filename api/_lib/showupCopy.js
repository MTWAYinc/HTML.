...
function buildConfirmationEmail({ leadName, meetingTimeFormatted, callLink, rescheduleLink, prepLink }) {
  const paragraphs = [
    ...
    "Before we talk, it'd help to know a bit about where your business is at right now, takes two minutes, use the link below.",
  ];
  const links = [
    { label: "Prep page", url: prepLink },
  ...
  return {
    subject: "You're confirmed with Taha at MTWAY",
    text,
    html: renderEmailHtml({
      heading: "You're confirmed.",
      paragraphs,
      primaryCta: { label: "Join The Call", url: callLink },
      links,
...
function buildReminderEmail({ leadName, reminderType, meetingTimeFormatted, callLink, rescheduleLink }) {
  ...
  const paragraphs = [`Hi ${firstName(leadName)},`, `Quick reminder, we're talking ${when} (${meetingTimeFormatted}).`];
  const links = [{ label: "Reschedule", url: rescheduleLink }];
  const text = [
    ...paragraphs,
    "",
    `Call link: ${callLink}`,
  ...
  return {
    subject: subjectByType[reminderType] || "Reminder: your call with Taha",
    text,
    html: renderEmailHtml({
      ...
      primaryCta: { label: "Join The Call", url: callLink },
      links,
      signatureLines: ["See you soon,", ...TAHA_SIGNATURE],
    }),
  };
}
...
function buildSindyCancellationNotice({ leadName, leadEmail, meetingTimeFormatted }) {
  ...
  return {
    subject: `Cancellation: ${leadName || leadEmail}`,
    text,
    html: renderEmailHtml({ heading: "Cancellation.", paragraphs, signatureLines: [] }),
  };
}
